...
namespace reflect {
    ...
    public:
        ...
        virtual Reference GetFieldByName(const Reference& o, const std::string& name) const = 0;

        virtual const std::vector<Field>& GetStaticFields() const = 0;
        virtual Reference GetStaticFieldByName(const std::string& name) const = 0;
...
reflect::Object::Object(T&& t)
        :    id_(GetTypeId<std::decay_t<T>>()), data_(new std::decay_t<T>(std::forward<T>(t))) {
